...
def custom_backward(output, grad_output, lbl_loss=None):
    ...
    if lbl_loss is not None:
        ...
        grad_lbl_loss = torch.ones_like(lbl_loss, memory_format = torch.preserve_format,)
...
def forward_step(
    ...
    if (
        ...
        return [output_tensor, input_tensor[-1], loss]
...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    if output_tensor[1] is not None and config.grad_scale_func is not None:
        output_tensor[1] = config.grad_scale_func(output_tensor[1])

    if config.deallocate_pipeline_outputs:
        # three cases: 1. last stage, 2. middle stage with lbl_loss, 3. middle stage without lbl_loss
        if parallel_state.is_pipeline_last_stage():
            custom_backward(output_tensor[0], output_tensor_grad[0])
        elif output_tensor[1] is not None:
        ...
        if parallel_state.is_pipeline_last_stage():
            ...
            torch.autograd.backward(output_tensor[1], grad_tensors=output_tensor_grad[1])
        else:
            torch.autograd.backward(output_tensor[0], grad_tensors=output_tensor_grad[0])
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    for i in range(num_warmup_microbatches):
        ...
        send_output_tensor = output_tensor[:-1]
        send_forward(send_output_tensor, send_tensor_shapes, config)
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            ...
            # )
            output_tensor_grad = send_forward_recv_backward(
                send_output_tensor, send_tensor_shapes, config
            )
